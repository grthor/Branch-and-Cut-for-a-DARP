...
//		}
		...
		try {
			...
			for (int j = 1; j <= 2*n; j++) {
				...
				cplex.addEq(expr, 1.0, "Constraint3");
			}

			// Constraint 4 Pesch : Visit pickup and dropdown location by the same vehicle.
			...
			// Constraint 5: Flow constraint: Every Node (V') except of 0
			// and 2*n+f+1 must have the same amount of ingoing and  
			// outgoing edges.
...
//			}
			...
			for (int i = 0; i < V.length; i++) {
				for (int k = 0; k < K.length; k++) {
					for (int r = 0; r <= 3; r++) {
						cplex.addLe(Math.max(0, V[i].getLoad()[r]), Q[i][r][k], "Constraint13_1");
						cplex.addLe(Q[i][r][k],
...
//			}
			...
			// Constraint 14 Pesch: Start with no container loaded.
			...
			// Definition L_i^k: Transportation time of a loaded container on k.
			...
			// Constraint 16: Presendence relation. Knoten i muss vor Knoten i+3 
			// besucht werden. 
			for (int k = 0; k < K.length; k++) {
				for (int i = 1; i <= n; i++) {
					IloLinearNumExpr expr = cplex.linearNumExpr();
					...
					cplex.addGe(B[i+n][k], expr, "Constraint16");
				}
			}
...
//							// Die 1440 sind der maximale Planungshorizont.
//							expr1.setConstant(t[i][j] + V[i].getServiceDuration() + 20000);
//							expr1.addTerm(-20000, x[i][j][k]);
//							cplex.addLe(B[j][k], expr1, "Constraint19a");
//							
//							IloLinearNumExpr expr2 = cplex.linearNumExpr();
//							expr1.addTerm(1.0, B[i][k]);
//							expr2.setConstant(t[i][j] + V[i].getServiceDuration() - 20000);
//							expr2.addTerm(20000, x[i][j][k]);
...
//			}
			...
			// Fahrzeit eines LKWs nicht überschreiten.
